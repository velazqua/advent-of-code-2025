...
int main () {
    ...
    // For the test input, we are looking at the 10 shortest distances. For part 1, we need to look at the first 1000.
    int LIMIT = 1000;
    ...
    // Find the largest 3 circuits and multiply their sizes to get the result.
    vector<int> circuit_sizes;
    for (const auto& entry : circuit_map) {
        circuit_sizes.push_back(entry.second.size());
    }
    sort(circuit_sizes.rbegin(), circuit_sizes.rend());
    int64_t result = 1;
    for(int i = 0; i < circuit_sizes.size(); ++i) {
        cout << "Circuit " << i << " size: " << circuit_sizes[i] << endl;
    }
    for (int i = 0; i < 3; ++i) {
